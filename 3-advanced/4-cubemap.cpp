...
int cubemap()
{
	...
	*/
	...
	while (!glfwWindowShouldClose(window)) // 渲染循环
	{
		...
		// 绘制天空盒
		glDepthFunc(GL_LEQUAL);  // 深度值等于深度缓冲中值时通过深度测试（天空盒深度已在4-skybox-vs.glsl中设为1）
		skyboxShader.use();
		view = glm::mat4(glm::mat3(camera.GetViewMatrix())); // 去掉view matrix的平移
