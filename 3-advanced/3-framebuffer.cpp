/*
* 帧缓冲
* 先渲染到自定义帧缓冲，再作为纹理渲染到屏幕上（或其它位置），便于实现反相、边缘检测等效果
* https://learnopengl-cn.github.io/04%20Advanced%20OpenGL/05%20Framebuffers/
* 
* 对帧缓冲（glGenFramebuffers）附加渲染缓冲还是纹理：
* 如果不需要从一个缓冲中采样数据，那么应该选择渲染缓冲（glGenRenderbuffers）。
* 如果需要从缓冲中采样颜色或深度值等数据，那么应该选择纹理（glGenTextures）。
*/
...
int framebuffer()
{
	...
	*/
	...
	// 将渲染缓冲（深度和模板）绑定到framebuffer（不需要采样）
	unsigned int rbo;
	...
	while (!glfwWindowShouldClose(window)) // 渲染循环
	{
		...
		screenShader.use();
		glBindVertexArray(quadVAO);
		glBindTexture(GL_TEXTURE_2D, textureColorbuffer);	// 使用color attachment texture
		glDrawArrays(GL_TRIANGLES, 0, 6);
